...
#include <gzpluginz/CustElevatorPlugin.h>
#include <gzpluginz/CustElevatorPluginPrivate.h>
...
void ElevatorPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf) {
  ...
  this->dataPtr->doorController =
      new ElevatorPluginPrivate::DoorController(this->dataPtr->doorJoint);
  ...
  this->dataPtr->elevatorSub = this->dataPtr->node->Subscribe(
      elevatorTopic, &ElevatorPlugin::OnElevator, this);
}
...
void ElevatorPlugin::MoveToFloor(const int _floor) {
  ...
  this->dataPtr->states.push_back(
      new ElevatorPluginPrivate::CloseState(this->dataPtr->doorController));
  ...
  this->dataPtr->states.push_back(
      new ElevatorPluginPrivate::OpenState(this->dataPtr->doorController));

  // Step 4: Wait
  this->dataPtr->states.push_back(
      new ElevatorPluginPrivate::WaitState(this->dataPtr->doorWaitTime));
...
bool ElevatorPluginPrivate::DoorController::Update(
  ...
  double doorError = this->doorJoint->Position() - errorTarget;

  double doorForce =
      this->doorPID.Update(doorError, _info.simTime - this->prevSimTime);
...
bool ElevatorPluginPrivate::LiftController::Update(
  ...
  double error =
      this->liftJoint->Position() - (this->floor * this->floorHeight);
